    ...
    fn infer_member_from_type(
        ...
        match obj_type {
            Type::Array(elem) => match property {
                "length" => return Ok(Type::Number),
                // Mutating methods
                "push" => return Ok(Type::simple_func(vec![*elem.clone()], Type::Number)),
                "pop" => return Ok(Type::simple_func(vec![], *elem.clone())),
                "reverse" => return Ok(Type::simple_func(vec![], Type::array(*elem.clone()))),
                "sort" => return Ok(Type::simple_func(vec![], Type::array(*elem.clone()))),
                "fill" => {
                ...
                "indexOf" => return Ok(Type::simple_func(vec![*elem.clone()], Type::Number)),
                "includes" => return Ok(Type::simple_func(vec![*elem.clone()], Type::Boolean)),
                "slice" => {
                    return Ok(Type::simple_func(
                ...
                "filter" => {
                    let callback = Type::simple_func(vec![*elem.clone()], Type::Boolean);
                    return Ok(Type::simple_func(
                        vec![callback],
                        Type::array(*elem.clone()),
                    ));
                ...
                "forEach" => {
                    let callback = Type::simple_func(vec![*elem.clone()], Type::Undefined);
                    return Ok(Type::simple_func(vec![callback], Type::Undefined));
                }
                "reduce" => {
                    let u = self.fresh_type_var();
                    let callback = Type::simple_func(vec![u.clone(), *elem.clone()], u.clone());
            ...
            Type::String => match property {
                "length" => return Ok(Type::Number),
                // String methods
                "indexOf" => return Ok(Type::simple_func(vec![Type::String], Type::Number)),
                "substring" => {
                    return Ok(Type::simple_func(
                        vec![Type::Number, Type::Number],
                        Type::String,
                ...
                "charAt" => return Ok(Type::simple_func(vec![Type::Number], Type::String)),
                "charCodeAt" => return Ok(Type::simple_func(vec![Type::Number], Type::Number)),
                "startsWith" => return Ok(Type::simple_func(vec![Type::String], Type::Boolean)),
                "endsWith" => return Ok(Type::simple_func(vec![Type::String], Type::Boolean)),
                "includes" => return Ok(Type::simple_func(vec![Type::String], Type::Boolean)),
                "repeat" => return Ok(Type::simple_func(vec![Type::Number], Type::String)),
                "padStart" => {
                    return Ok(Type::simple_func(
...
mod tests {
    ...
    fn test_annotation_more_specific_than_inferred() {
        ...
        let (_, env, state) =
            infer_program_with_state("/** var x: Number[] */ var x = [];").unwrap();
        let scheme = env.lookup("x").unwrap();
        let ty = state.apply_subst(&scheme.body.ty);
        assert_eq!(ty, Type::array(Type::Number));
    }
    ...
    fn test_array_map() {
        let (_, env, state) =
            infer_program_with_state("var result = [1, 2, 3].map(function(x) { return x + 1; });")
                .unwrap();
    ...
    fn test_array_foreach() {
        let ty = infer_expr_str("[1, 2, 3].forEach(function(x) { console.log(x); })").unwrap();
        assert_eq!(ty, Type::Undefined);
    }

    #[test]
    fn test_array_some() {
        let ty = infer_expr_str("[1, 2, 3].some(function(x) { return x > 2; })").unwrap();
        assert_eq!(ty, Type::Boolean);
    }

    #[test]
    fn test_array_every() {
        let ty = infer_expr_str("[1, 2, 3].every(function(x) { return x > 0; })").unwrap();
        assert_eq!(ty, Type::Boolean);
    }

    #[test]
    fn test_array_find() {
        let ty = infer_expr_str("[1, 2, 3].find(function(x) { return x > 1; })").unwrap();
        assert_eq!(ty, Type::Number);
    }
    ...
    fn test_array_reduce() {
        let ty =
            infer_expr_str("[1, 2, 3].reduce(function(acc, x) { return acc + x; }, 0)").unwrap();
